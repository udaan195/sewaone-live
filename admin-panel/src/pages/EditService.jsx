...
export default function EditService({ onLogout }) {
  ...
  if(loading) return <div className="dashboard-container"><p style={{padding:20}}>Loading Data...</p></div>;
  ...
  return (
    <div className="dashboard-container">
      ...
      <main className="dashboard-main">
        <Link to="/services/manage" className="back-link">&larr; Back to List</Link>
        
        <div className="content-card form-card">
          <div style={{display:'flex', justifyContent:'space-between'}}>
              <h2 className="card-title">Edit: {formData.title}</h2>
              <span style={{padding:'5px 10px', background:'#eff6ff', color:'#2563eb', borderRadius:5, fontSize:12}}>{formData._id}</span>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="form-actions"><button type="submit" className="submit-btn" disabled={saving}>{saving?'Updating...':'Update Service'}</button></div>
          </form>
        </div>
      </main>
    </div>
